...
var speed = round_to_decimal(rng.randf_range(10, 250) * Global.enemy_difficulty, 0)  # Speed of movement to the right
var steal_value = round_to_decimal(rng.randf_range(0, 35) * Global.enemy_difficulty, 0)  # the amount the unit steals from the hous
var attack_cooldown = round_to_decimal(rng.randf_range(0.5, 2), 2)    # Time between attacks
var attack_damage = round_to_decimal(rng.randf_range(5, 20) * Global.enemy_difficulty, 0)   # Default attack damage
var health = round_to_decimal(rng.randf_range(30, 150) * Global.enemy_difficulty, 0)   # Unit health
var can_attack = true
